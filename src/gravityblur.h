 ...
 */
typedef struct {
    int type_in, type, type_out, flags, num_iter, info, ris;
    float range, sharp;
} gb_opts_t;

static void gravityblur(bitmap_t *in, bitmap_t *out, gb_opts_t *opts)
{
    ...
    int iter, j, x, y;
    float c0 = opts->sharp, c1 = c0 * sqrtf(0.5f);
...
#define CORE(i, x, y) { M3(d, x, y) \
    float t0 = b0 - d0; \
    t = opts->range - fabsf(t0); \
    if (t < 0) t = 0; t *= t; aw = c##i * t; \
    a0 += t0 * t * aw; an += aw * aw; }

    for (iter = 0; iter < opts->num_iter; iter++)
    {
...
static void gravityblur3(bitmap_t *in, bitmap_t *out, gb_opts_t *opts)
{
    uint8_t *in_data = in->data, *out_data = out->data, *temp;
    int w = in->width, h = in->height, bpp = in->bpp;
    int stride = in->stride, ostride = out->stride;
...
#define CORE(i, x, y) { M3(d, x, y) \
    ...
    t = opts->range - sqrtf(t0*t0 + t1*t1 + t2*t2); \
...
static void gravityblur4(bitmap_t *in, bitmap_t *out, gb_opts_t *opts)
{
    uint8_t *in_data = in->data, *out_data = out->data, *temp;
    int w = in->width, h = in->height, bpp = in->bpp;
    int stride = in->stride, ostride = out->stride;
